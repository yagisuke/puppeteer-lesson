...
(async () => {
  ...
  console.log('[指名OK][メンズカットが得意][女性スタイリスト]で絞り込みました------------------------------');

  const pageSize = await page.$eval('#mainContents .pageControl', item => {
    return Number(item.innerHTML.match(/\d+ページ/)[0].replace('ページ', ''));
  });
  console.log(`これから${pageSize}ページのスタイリスト情報を取得していきます------------------------------`);

  let result = [];

  for (let i = 1; i <= pageSize; i++) {
    console.log(`${i}ページ目のスタイリスト情報を取得中です------------------------------`);
    const stylists = await page.evaluate(selector => {
      return Array.from(document.querySelectorAll(selector), item => {
        return {
          ...
          url: item.querySelector('.slcHeadContentsInner a').href.replace(/\?.*$/, ''),
  ...
  console.log(`こちらの情報は${path}に保存しました------------------------------`);
  await browser.close();
})();
